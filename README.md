...
Packages used: `pygame, plyer, requests, Pillow, pyperclip, vdf` (+ `PyGObject` for Linux)
...
Alternatively, a `UserGameStatsSchema_[AppID].bin` file can be used instead of `achievements.json` and `stats.txt`. `achievement_images` can be downloaded after running the program.

If `games/steam_path.txt` is correct and the schema file for a given game is present in `Steam/appcache/stats`, it will be copied automatically. By default, `generator_path` has priority over this method even if the schema file exists. Disable `generator_priority` if you prefer using schema files whenever possible.
...
| Steam (local) | `steam_local` | `stl, steaml, steam_local`     | No "Achievement Progress" notifications |
...
An alternative way of tracking achievements from Steam that relies on local files.

Differences:

- Uses a different format for user IDs, but conversion from the usual format is done automatically.

- `games/steam_path.txt` must contain the correct path to your Steam installation.

- Requires no Steam Web API key.

- Can be used even if Steam is in offline mode or your achievements are private.

- Timestamps for new unlocks show the actual unlock time, not the value saved to your Steam profile. Usually just a few seconds of difference, but connection problems can make it worse.
...
`generator_priority` - prioritize using `generator_path` (if it's set) over copying UserGameStatsSchema files from `Steam/appcache/stats`. Default: `true`
...
Settings used: `language`, `language_requests`, `savetime_shown`, `bar_hide_secret`, `bar_ignore_min`, `unlockrates`, `secrets`, `hidden_title`, `hidden_desc`, `stat_display_names`, `show_timestamps`, `strftime`, `api_key` (if loading progress from Steam)
