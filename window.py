                    ...
                    if ach.progress.support:
                        if not (stg['bar_unlocked'] in ('full', 'zerolen') and ach.earned):
                            ...
                            if stg['bar_ignore_min'] or ach.progress.min_val == ach.progress.max_val:
                                prg_no_min = (ach.progress.real_value, ach.progress.max_val)
                        else:
                            prg_no_min = (1, 1)
                    else:
                        prg_no_min = (0, 1)
...
def load_everything(reload=False, keep_data=False):
    ...
    if stg['rare_below_relative'] or stg['rare_guaranteed']:
        ...
        if stg['rare_below_relative'] and len(sorted_urates) > 0:
            ...
            stg['rare_below_relative'] = False # To prevent multiplying again after Ctrl+Shift+R
